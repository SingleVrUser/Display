...
internal class FileUploadService : UploadBase
{
    ...
    private AliyunOssService _aliyunOssService;
    ...
    {
        if (_aliyunOssService == null)
        {
            _aliyunOssService = new AliyunOssService(Client, _stream, fastUploadResult, progress: new Progress<long>(
                p =>
                {
                    Position = p;
            ...
            _aliyunOssService.StateChanged += state =>
            {
        ...
        await _aliyunOssService.Init(_source.Token);
    ...
    {
        var result = await _aliyunOssService.Start(token);
